...
async def events_list(request: Request) -> HTMLResponse | RedirectResponse:
    ...
    async with SessionLocal() as session:
        stmt = select(Event).options(selectinload(Event.groups)).order_by(Event.date.desc())
...
async def event_cover_upload(request: Request, event_id: int, file: UploadFile) -> RedirectResponse:
    user = await require_permission(request, StaffPermission.events)
    if user is None:
        return login_redirect()
    async with SessionLocal() as session:
        ...
        try:
            ...
            return RedirectResponse(f"/admin-tools/events/{event_id}/edit?flash_error={exc}", 303)
